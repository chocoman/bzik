...
Mat flatten( Mat img )
{
	...
	for(int x=0;x<16;x++)
	{
		for(int y=0;y<16;y++)
		{
			data.at<float>(0,i) = (img.at<uchar>(x,y) == 255) ? 1 : 0;
			i++;
		}
	}
	return data;
...
char recognize( Mat s, double *confidence ){
	int maxIndex = 0;
	int secondIndex = 0;
	cv::Mat classOut(1, symbolsSize, CV_32F);
	nnetwork.predict(flatten(s), classOut);
	float value;
	float maxValue = -1;
	float secondValue = -1;
	for( int index=0; index<symbolsSize; index++ ){
	...
	*confidence = maxValue - secondValue;
...
char humanRecognize(Mat s, double *confidence){ //not used
	char character;
...
bool analyze( Mat image ) {
	...
	*/
	...
	if (!origins.size()){
		cout << "origins empty, stopping" << endl;
		return false;
	}
...
int main ( int argc,char **argv ) {
	if (argc == 11){
		...
		CONFIDENCE_THRESHOLD = std::stof(argv[10]);
		...
		cout << "CONFIDENCE_THRESHOLD" << CONFIDENCE_THRESHOLD << endl;
	} else {
		cout << "no arguments provided, using default parameters" << endl;
		MAX_SIZE = 40;
		MIN_SIZE = 5;
		THRESHOLD_LIGHT = 30;
		MIN_DISTANCE = 20;
